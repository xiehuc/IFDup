...
#endif
      if (ReturnInst *RI = dyn_cast<ReturnInst>(LastCond)) {
...
#ifdef R_DEBUG
            errs() << "SetupBr-setxx:BB{"<< BI->getParent()->getName()<<"} ";
#endif
...
#endif
         if (removeOverlapOnValue((*keyi),checkedAt, PDT))
            changed = true;
#ifdef R_DEBUG
         //errs() <<"\n";
#endif
...
bool
RedundAnalysis::removeOverlapOnValue(Value*v,ValueCheckedAt* checkatTable,PostDominatorTree &PDT) 
{
   bool changed = false;
   ...
   if (!(checkatTable->notRemovable()) ) {
      ...
      for (i = CheckedAtList.begin(), ie = CheckedAtList.end(); i != ie; i++) {
         ...
         if (PDT[hereMycheck->getParent()] == NULL) continue;
         ...
         for (j = PropOrFinalList.begin(), je = PropOrFinalList.end(); j!=je; j++) {
            ...
            // FIXME : xiehuc couldn't safely replace PostDominatorSet
            if (PDT.dominates(latercheck->getParent(), hereMycheck->getParent())) 
                  ...
                  if (PDT.dominates(latercheck->getParent(), hereMycheck->getParent())) 
                     if (!hasSynchPoint(hereMycheck,latercheck))  {
                        ...
                        if (reg_safe) {
                           ...
                           if (!(DT->dominates(hereMycheck, latercheck))) {
                              erased = true;
                              break;
                           }
                           //end REG_SAFE
...
void
RedundAnalysis::enableCheckADVRegSafe(DominatorTree *dominset) {
   reg_safe = true;
