...
void ParIFDuplica::getAnalysisUsage(AnalysisUsage &AU) const
{
   AU.addRequired<DominatorTree>();
   AU.addRequired<ShortcutDetectorPass>();
}
bool ParIFDuplica::runOnFunction(Function &F) 
{
   localnumreplicatedBB = 0;
   ShortcutDetectorPass& SCDetectorPass = getAnalysis<ShortcutDetectorPass>();
...
BasicBlock *ParIFDuplica::buildErrorBlock(Function &F) {
   ...
   std::string callName = "";
   CallInst::Create(callfun, const_23, callName, EB); /* newCall */
   ...
   */
   ...
   new UnreachableInst(C, EB); /* endinst */
   return EB;
}
