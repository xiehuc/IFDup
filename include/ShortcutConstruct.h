...
namespace llvm {
 ...
 public:
    ...
    public:
     ...
     bool isnoRep() {return (isPRepEmpty() && isFRepEmpty());}
 ...
 ////////////////////////////////
 //struct DominatorSet; 
static  Statistic<> NumShortcut("numshortcut", "Number of shortcut branches detected");
static Statistic<> NumShortcutSet("numshortcutset", "Number of shortcut branche SETs detected");
static Statistic<> NumFailDomTest("numfaildomtest","Number of set failed domination test");
 static Statistic<> NumWorkNodes("numworkBB", "Number of BBs we are working on");
  class ChildrenSet {
  public:
     ChildrenSet(BasicBlock *thisBB, BasicBlock *leftleaf, BasicBlock*rightleaf);
     ChildrenSet(BasicBlock *thisBB, BasicBlock *leftleaf, ChildrenSet *rightSet, DominatorSet *dominset);
     ChildrenSet(BasicBlock *thisBB, ChildrenSet *leftSet, BasicBlock *rightleaf,DominatorSet *dominset );
     ChildrenSet(BasicBlock *thisBB, ChildrenSet *leftSet, ChildrenSet *rightSet,DominatorSet *dominset );
    inline ~ChildrenSet();
    ...
    private:
      ...
      ChildrenSet * isShortcut(BasicBlock *child1, ChildrenSet *childset2, int *lastLeft, BasicBlock *upBB,DominatorSet *dominset );
  ...
  public:
    ...
    //////////////////////////////////////
    //////////////////////////////////////
 class ShortcutDetectorPass {
   public:
     std::list<ChildrenSet*> *runOnFunction(Function &, DominatorSet&);
   ...
   private:
     ...
     bool havePHI(BasicBlock *BB);
     int localshortcut, localSCset, localFailed, localWorkNodes;
     DominatorSet *dominset;
     void conSCSetMap(std::set<BasicBlock*>&, std::set<BasicBlock*>& ,std::map<BasicBlock*,ChildrenSet*>&);
     void BuildHeadNodeList (std::map<BasicBlock*,ChildrenSet*>&, Function &, std::list<ChildrenSet*>&);
